...
class KitDinamiteBot:
    ...
    def vender_kit_dinamite(self):
        """Gerencia a venda do kit dinamite (dinamite)"""
        try:
            # Verifica se está na loja
            if state_manager.get_current_state() != "Dentro da Loja":
                raise Exception("Bot não está dentro da loja!")
            
            # Define a sequência de vendas (5 caixas com dinamite)
            sequencia_vendas = [
                {"caixa": f"box{i}", "item": "dinamite", "quantidade": 10} for i in range(1, 6)
            ]
            ...
            print(f"Erro ao gerenciar venda de kit dinamite: {e}")
            return f"Erro: {str(e)}"
    ...
    def vender_item(self, box_name, item_name, quantidade):
        ...
        try:
            ...
            if encontrou:
                ...
                if not encontrou:
                    print(f"Quantidade incorreta para {item_name}, tentando aumentar...")
                    # Tenta aumentar a quantidade
                    screenshot = self.capture_screen()
                    encontrou, centro = self.encontrar_template_na_tela(
                        screenshot, 
                        template_name="quantiaMais"
                    )
                    if encontrou:
                        self.clicar_elemento(centro)
                        time.sleep(self.config["delays"]["after_click_quantidade"])
                        ...
                        if not encontrou:
                            print(f"Conta está sem {item_name} suficiente mesmo após aumentar")
